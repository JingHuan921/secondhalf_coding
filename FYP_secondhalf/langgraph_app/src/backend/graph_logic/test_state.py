...
def _create_versioned_artifact(original_artifact: Artifact, new_version: str) -> Artifact:
    ...
    return Artifact(
        id=f"{original_artifact.id}_v{new_version.replace('.', '_')}",  # Unique ID with version
...
def create_artifact(
    ...
    content: Optional[List[Union[RequirementsClassificationList, SystemRequirementsList, RequirementsModel]]] = None,
