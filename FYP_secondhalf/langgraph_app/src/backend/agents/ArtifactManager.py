...
class ArtifactManager:
    ...
    def get_latest_artifact(state: AgentState, artifact_type: str) -> Optional[Artifact]:
        ...
        if not artifacts:
            return None
        return max(artifacts, key=lambda x: x.metadata.version)
...
def archivist_agent(state: AgentState) -> dict:
    ...
    srl_info = ""
    if srl_artifact:
        srl_info = f"Using SRL version {srl_artifact.metadata.version} from analyst"
    ...
    if validation_artifact and isinstance(validation_artifact.content, BaseModel):
        validation_feedback = f"Incorporating feedback from validation report v{validation_artifact.metadata.version}"
...
def reviewer_agent(state: AgentState) -> dict:
    ...
    rm_info = ""
    if rm_artifact:
        rm_info = f"Also reviewed RM version {rm_artifact.metadata.version}"
